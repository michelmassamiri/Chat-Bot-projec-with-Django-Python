...
def import_data(request):
    if request.method == "POST":
        ...
        def choice_func(row):
            q = Person.objects.filter(slug=row[0])[0]
            row[0] = q
            return row
        if form.is_valid():
            request.FILES['file'].save_to_database(
                ...
                model=Person,
                initializer=None,
                mapdict=['nom', 'contact', 'role', 'responsable', 'niveauH', 'equipe', 'competences', 'projets', 'complements']
                )
...
def survey_result(request):
    person = Person.objects.get(slug='ide')
    query_sets = Choice.objects.filter(question=question)
